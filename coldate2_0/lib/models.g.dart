...
class TableTodo extends SqfEntityTableBase {
  TableTodo() {
    ...
    // declare fields
    fields = [
      SqfEntityFieldBase('cal', DbType.integer, isNotNull: false),
      SqfEntityFieldBase('date', DbType.text, isNotNull: false),
      SqfEntityFieldBase('year', DbType.text, isNotNull: false),
    ];
    super.init();
  }
...
// region Todo
class Todo {
  Todo({this.id, this.cal, this.date, this.year, this.isDeleted}) {
    _setDefaultValues();
  }
  Todo.withFields(this.cal, this.date, this.year, this.isDeleted) {
    _setDefaultValues();
  }
  Todo.withId(this.id, this.cal, this.date, this.year, this.isDeleted) {
    _setDefaultValues();
  }
  Todo.fromMap(Map<String, dynamic> o, {bool setDefaultValues = true}) {
    ...
    if (o['cal'] != null) {
      cal = int.tryParse(o['cal'].toString());
    }
    if (o['date'] != null) {
  ...
  List<dynamic> toArgs() {
    return [cal, date, year, isDeleted];
  }

  List<dynamic> toArgsWithIds() {
    return [id, cal, date, year, isDeleted];
  }
  ...
  Future<List<dynamic>> saveAll(List<Todo> todos) async {
    // final results = _mnTodo.saveAll('INSERT OR REPLACE INTO todo (id,cal, date, year,isDeleted)  VALUES (?,?,?,?,?)',todos);
    // return results; removed in sqfentity_gen 1.3.0+6
    TodoDbModel().batchStart();
    for (final obj in todos) {
      await obj.save();
    }
    return TodoDbModel().batchCommit();
  }
  ...
  Future<int> upsert() async {
    try {
      if (await _mnTodo.rawInsert(
              'INSERT OR REPLACE INTO todo (id,cal, date, year,isDeleted)  VALUES (?,?,?,?,?)',
              [id, cal, date, year, isDeleted]) ==
          1) {
...
class TodoFilterBuilder extends SearchCriteria {
  ...
  TodoField _cal;
  TodoField get cal {
    return _cal = setField(_cal, 'cal', DbType.integer);
  }

  TodoField _date;
  TodoField get date {
    return _date = setField(_date, 'date', DbType.text);
  }
...
class TodoFields {
  ...
  static TableField _fCal;
  static TableField get cal {
    return _fCal = _fCal ?? SqlSyntax.setField(_fCal, 'cal', DbType.integer);
  }
